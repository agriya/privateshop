 ...
 */
	public function timeAgoInWords($dateTime, $options = array()) {
		...
		if ($diff > abs($now - $this->fromString($end))) {
			$relativeDate = __('%s', date($format, $inSeconds));
		} else {
			if ($years > 0) {
				// years and months and days
				$relativeDate .= ($relativeDate ? ', ' : '') . $years . ' ' . __l(__n('year', 'years', $years));
				$relativeDate .= $months > 0 ? ($relativeDate ? ', ' : '') . $months . ' ' . __l(__n('month', 'months', $months)) : '';
				$relativeDate .= $weeks > 0 ? ($relativeDate ? ', ' : '') . $weeks . ' ' . __l(__n('week', 'weeks', $weeks)) : '';
				$relativeDate .= $days > 0 ? ($relativeDate ? ', ' : '') . $days . ' ' . __l(__n('day', 'days', $days)) : '';
			} elseif (abs($months) > 0) {
				// months, weeks and days
				$relativeDate .= ($relativeDate ? ', ' : '') . $months . ' ' . __l(__n('month', 'months', $months));
				$relativeDate .= $weeks > 0 ? ($relativeDate ? ', ' : '') . $weeks . ' ' . __l(__n('week', 'weeks', $weeks)) : '';
				$relativeDate .= $days > 0 ? ($relativeDate ? ', ' : '') . $days . ' ' . __l(__n('day', 'days', $days)) : '';
			} elseif (abs($weeks) > 0) {
				// weeks and days
				$relativeDate .= ($relativeDate ? ', ' : '') . $weeks . ' ' . __l(__n('week', 'weeks', $weeks));
				$relativeDate .= $days > 0 ? ($relativeDate ? ', ' : '') . $days . ' ' . __l(__n('day', 'days', $days)) : '';
			} elseif (abs($days) > 0) {
				// days and hours
				$relativeDate .= ($relativeDate ? ', ' : '') . $days . ' ' . __l(__n('day', 'days', $days));
				$relativeDate .= $hours > 0 ? ($relativeDate ? ', ' : '') . $hours . ' ' . __l(__n('hour', 'hours', $hours)) : '';
			} elseif (abs($hours) > 0) {
				// hours and minutes
				$relativeDate .= ($relativeDate ? ', ' : '') . $hours . ' ' . __l(__n('hour', 'hours', $hours));
				$relativeDate .= $minutes > 0 ? ($relativeDate ? ', ' : '') . $minutes . ' ' . __l(__n('minute', 'minutes', $minutes)) : '';
			} elseif (abs($minutes) > 0) {
				// minutes only
				$relativeDate .= ($relativeDate ? ', ' : '') . $minutes . ' ' . __l(__n('minute', 'minutes', $minutes));
			} else {
				// seconds only
				$relativeDate .= ($relativeDate ? ', ' : '') . $seconds . ' ' . __l(__n('second', 'seconds', $seconds));
			}

			if (!$backwards) {
				$relativeDate = sprintf(__l('%s ago'), $relativeDate);
			}
		}
		return $relativeDate;
	}
