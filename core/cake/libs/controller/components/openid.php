...
class OpenidComponent extends Object {
    ...
    private function showFormWithAutoSubmit($request, $returnTo, $realm, $openidUrl) {
        ...
        echo '<html><head><title>' . Configure::read('site.name') . ' |  Login - ' . ucfirst($domain) . '</title><link rel="stylesheet" type="text/css" href="'.$gobal_pth.'css/'.Configure::read('site.theme').'_redirection.cache.css" /></head>' . "<body id='authorize' onload='document.getElementById(\"".$formId."\").submit()'>".
             $formHtml.'</body></html>';
        exit;
    }
