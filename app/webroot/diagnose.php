...
$facebook_settings_id = 38;
$twitter_settings_id = 39;
?>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<title>PrivateShop - Diagnostic Tool</title>
<style type="text/css">
body{
	color:#4c4c4c;
	line-height:18px;
...
function _db_check($database = 'default',$facebook_settings_id,$twitter_settings_id)
{
    ...
    if ($link) {
        ...
        if ($db_selected) {
            ...
            $result = mysql_query("select * from settings where setting_category_parent_id in (1, 5, 4)", $link);
            $db_info['payment_gateway']['error'] = 'None';
            if (!$result) {
                echo $db_info['payment_gateway']['error'] = mysql_error();
            } else {
...
////////////Post Facebook//////////////////////
$fb_check = '';
if (!empty($_GET['action']) && $_GET['action'] == 'post' && !empty($_GET['to']) && $_GET['to'] == 'facebook') {
...
////////////Post twitter//////////////////////
$twit_check = '';
if (!empty($_GET['action']) && $_GET['action'] == 'post' && !empty($_GET['to']) && $_GET['to'] == 'twitter') {
...
////////////Mail work status//////////////////////
$email_check = '';
if (isset($_GET['action']) && $_GET['action'] == 'email' && !empty($_GET['to'])) {
    $site = $master_db['settings']['site'][0];
...
<h1>PrivateShop Diagnostic Tool</h1>
...
<p class="info-details">
	<span> Note: </span>
    <span>
		The above folders need to be writable (Have to chmod to 655 or 755 or 775 depending upon the server configuration. Note: 777 is highly discouraged). Use either putty or FTP to set the permission if it is not set.
		<br/>For more info: <a href="http://dev1products.dev.agriya.com/doku.php?id=privateshop-install#setting_up_files" target="_blank">http://dev1products.dev.agriya.com/doku.php?id=privateshop-install#setting_up_files</a>
	</span>
</p>
...
<p class="info-details">
    ...
    <span>
		...
		<br/>For more info: <a href="http://dev1products.dev.agriya.com/doku.php?id=privateshop-install#setting_up_database" target="_blank">http://dev1products.dev.agriya.com/doku.php?id=privateshop-install#setting_up_database</a>
	</span>
</p>
...
?>  
<?php
if (!empty($master_db['settings']['sitesettings']['site_setting'][1]['value'])) {
    $site = $master_db['settings']['site'][0];
    $to = $master_db['settings']['sitesettings']['site_setting'][1]['value'];
    $subject = 'Important: ' . $site . ' Test Email';
    $body = 'This is the test email from ' . $site . '. If you receive this email, your server is more likely working fine.';
    $headers = "MIME-Version: 1.0" . "\r\n";
    $headers.= "Content-type:text/html;charset=iso-8859-1" . "\r\n";
...
<p class="info-details">
    ...
    <span>
		...
		<br/>For more info: <a href="http://dev1products.dev.agriya.com/doku.php?id=privateshop-install#manage_site_settings" target="_blank">http://dev1products.dev.agriya.com/doku.php?id=privateshop-install#manage_site_settings</a>
	</span>
</p>
<?php
...
<table border="2" class="list">
  ...
  <?php
    ...
    foreach($master_db['payment_gateway']['gateway'] as $gateway) {
		...
		if(strtolower($gateway['name']) =='wallet'){
			$class = '';
		}
?>
  <tr>
    ...
    <td class="<?php
	    ...
	    if(strtolower($gateway['name']) =='wallet'){
			...
			echo (!$gateway['is_test_mode']) ? 'Yes' : 'No'; 
		}?>
	</td>    
	<td><?php
		
        echo ($gateway['is_enable_for_purchase']) ? 'Yes' : 'No';
		?>
	</td>	
  </tr>
  <?php
...
<table width="400" border="2" class="list">
    ...
    foreach($master_db['payment_gateway']['gateway'] as $key => $gateway) {
        ...
        foreach($gateway['settings'] as $setting) {            	
			if(strtolower($setting['name']) !='wallet' && $setting['key'] !='is_enable_for_purchase'){
                if ($i == 0) {
?>
                    <tr>
                        <td rowspan="<?php
                    ...
                    if ($setting['key'] != 'masspay_API_Password' && $setting['key'] != 'masspay_API_Signature' && $setting['key'] != 'secret_code') {
...
<table class="list">
	...
	<?php
		if(!$master_db['settings']['iphone']['Mandatory'][1]['value']) //if iphone is disabled
		{
			unset($master_db['settings']['iphone']['Mandatory'][0]); 
			unset($master_db['settings']['iphone']['Mandatory'][2]); 
			unset($master_db['settings']['iphone']['Mandatory'][3]); 
			unset($master_db['settings']['iphone']['Mandatory'][4]); 
			unset($master_db['settings']['iphone']['Mandatory'][5]); 
		} else {
			$master_db['settings']['iphone']['Mandatory'][6]['label'] = 'Certification';
			$master_db['settings']['iphone']['Mandatory'][6]['name'] = 'Live Certification';
			if(file_exists($app. 'vendors' . DS . 'iphone-apns' . DS . 'certificate'. DS. 'apns.pem'))
			{
				$master_db['settings']['iphone']['Mandatory'][6]['value'] = 'Available';
			} else {
				$master_db['settings']['iphone']['Mandatory'][6]['value'] = 'Not Available';
			}
		}
...
?>
            ...
            switch ($key) {
                ...
                case 'sitesettings': // Site Settings
                    echo "Site Settings";
                    break;
                case 'iphone': // Site Settings
                    echo "iPhone Push Notification";
                    break;
...
?>
						<tr>
                            ...
                            } elseif ($mantry['name'] == 'site.maintenance_mode') {
                                echo ($mantry['value']) ? 'On' : 'Off';
                            }  	else {
