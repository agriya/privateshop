 ...
 */
	...
	//$view_cache = CACHE.DS."views".DS."user".DS.'privateshopdev_dev_agriya_com'."/".$this->Auth->user('id')."/_payments_order_en.gz";
    //cache_clear($view_cache);
...
function permanentCached($requested_url = null) {
	...
	if (empty($debug_mode) && PERMANENT_CACHE_CHECK && empty($_COOKIE['_flash']) && !in_array($_SERVER['REQUEST_METHOD'], array('POST', 'PUT', 'DELETE')) && !empty($_SERVER['HTTP_ACCEPT_ENCODING']) && strpos($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip') !== false) {
		...
		if (!empty($gzip_user_id)) {
			...
			$payment_cache = '..' . DS . 'tmp' . DS . 'cache' . DS . "views".DS."user".DS. str_replace('.', '_', $_SERVER['HTTP_HOST']) . DS . $gzip_user_id."/_payments_order_en.gz";
			$money_transfer_cache = '..' . DS . 'tmp' . DS . 'cache' . DS . "views".DS."user".DS. str_replace('.', '_', $_SERVER['HTTP_HOST']) . DS . $gzip_user_id."/_money_transfer_accounts_add_requested_en.gz";
			$cash_widthdraws_cache = '..' . DS . 'tmp' . DS . 'cache' . DS . "views".DS."user".DS. str_replace('.', '_', $_SERVER['HTTP_HOST']) . DS . $gzip_user_id."/_user_cash_withdrawals_en.gz";
			$dash_board_cache = '..' . DS . 'tmp' . DS . 'cache' . DS . "views".DS."user".DS. str_replace('.', '_', $_SERVER['HTTP_HOST']) . DS . $gzip_user_id."/_users_dashboard_en.gz";
			$multiple_address_cache = '..' . DS . 'tmp' . DS . 'cache' . DS . "views".DS."user".DS. str_replace('.', '_', $_SERVER['HTTP_HOST']) . DS . $gzip_user_id."/_payments_order_type_multiple_en.gz";
			$login_image_cache = '..' . DS . 'tmp' . DS . 'cache' . DS . "views".DS."user".DS. str_replace('.', '_', $_SERVER['HTTP_HOST']) . DS . $gzip_user_id."/_admin_users_customise_force_login_en.gz";
            cache_clear($payment_cache);
            cache_clear($money_transfer_cache);
            cache_clear($cash_widthdraws_cache);
            cache_clear($dash_board_cache);
            cache_clear($multiple_address_cache);
            cache_clear($login_image_cache);
		} else {
