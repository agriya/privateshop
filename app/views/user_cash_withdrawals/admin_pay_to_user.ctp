    ...
    ?>
        <tr<?php echo $class;?>>
            ...
            <td class="dl"><?php echo $this->Form->input('UserCashWithdrawal.'.($i-1).'.gateways',array('type' => 'select', 'options' => $userCashWithdrawal['paymentways'], 'label' => false, 'class' => "js-payment-gateway_select {container:'js-info-".($i-1)."-container'}")); ?>
            	...
            	<?php echo $this->Form->input('UserCashWithdrawal.'.($i-1).'.info',array('type' => 'textarea', 'label' => false, 'help' => 'Info for Paid')); ?>
                </div>
            </td>
                           <td class="dr"><?php echo $this->Html->siteCurrencyFormat($this->Html->cCurrency(!empty($userCashWithdrawal['User']['total_amount_withdrawn'])?$userCashWithdrawal['User']['total_amount_withdrawn']:0)); ?></td>
        </tr>
