...
class XAjaxComponent extends Component
{
    ...
    {        		
        ...
        header('Content-type: '.$_SESSION['product_file_info'][$file_id]['type']);
        header('Content-Length: ' . strlen($_SESSION['product_file_info'][$file_id]['thumb']));
        echo base64_decode($_SESSION['product_file_info'][$file_id]['thumb']);
        exit(0);
    }
	...
	function previewImage()
    {
        ...
        $_data['type'] = get_mime($_FILES['Filedata']['tmp_name']);
        $_data['filename'] = $_FILES['Filedata']['name'];
        $_data['thumb'] = base64_encode($imagevariable);
		...
		$this->Session->write("product_file_info.".$file_id, $_data);
        exit;
    }
        ...
        if ($multiple) {
            foreach($data['Attachment'] as $attachment) {
                ...
                if (!empty($attachment['filename']['name']) && $controller->{$modelClass}->save($data, false)) {
                    $attachments = array();
					$attachments['Attachment']['description'] = $attachment['description'];
